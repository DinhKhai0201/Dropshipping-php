...
class vendor_aws {
  ...
  public function createWithDate($file_name, $temp_file_location, $folder = null, $isOrigin = null){
    $result = $this->s3->putObject([
      'Bucket' => $this->bucket_name,
      'Key'    => ($folder?$folder.'/':"").date("Y/m/d").'/'.($isOrigin?'origin_':"").str_replace('/','-',date("Y/m/d")).'-'.time().rand(10000,99999).$file_name,
      'SourceFile' => $temp_file_location			
    ]);
  ...
  public function createBase64ResizeImage($dataBase64, $folder = null, $width = 200){
    ...
    $link = self::createWithDate($fileName, $fullUrl, $folder);
    $linkOrigin = self::createWithDate($fileName, $ulfd.'origin_'.$nameDate.'-'.$fileName, $folder, true);
    unlink($fullUrl);
  ...
  public function createResizeImage($file_name, $temp_file_location, $folder = null, $width = 200){
      ...
      $link = self::createWithDate($file_name, $ulfd.$newfn, $folder);
      $linkOrigin = self::createWithDate($file_name, $ulfd.'origin_'.$newfn, $folder, true);
      unlink($ulfd.$newfn);
